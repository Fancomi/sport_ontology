...
用法:
  ...
  python3 audit_videos.py $VLM [--workers 32] [--poll 30]

逻辑:
  - 每 --poll 秒扫描 videos/ 目录，找出未审核的完整视频（非 .part）
  - 抽中位帧 base64 → VLM 图像审核（复用 filter_vlm 的 prompt）
  - 通过: 记录进度，保留视频
...
def judge_video(video_path: Path, item: dict, client: LLMClient) -> tuple[bool, str]:
    """抽帧 + VLM 判断，返回 (passed, reason)"""
...
def run(workers: int, poll: int, client: LLMClient):
    ...
    while True:
        ...
        with ThreadPoolExecutor(max_workers=workers) as pool:
            futs = {pool.submit(judge_video, p, meta_map.get(p.stem, {}), client): p
                    for p in pending}
...
def main():
    ...
    run(args.workers, args.poll, client)
