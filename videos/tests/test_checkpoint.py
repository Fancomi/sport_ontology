...
TENNIS_IDENTITY = {
    ...
    "policy_version": "court-match-tennis-v2-loosecam",
}
...
def test_load_latest_identities_reads_last_record_per_item(tmp_path):
    ...
    records.write_text(
        ...
        '{"item":"a.mp4","passed":true,"reason":"","domain":"tennis",'
        '"schema_version":"court-match-v1","policy_version":"court-match-tennis-v2-loosecam"}\n'
    ...
    assert identities["a.mp4"]["policy_version"] == "court-match-tennis-v2-loosecam"
    assert identities["b.mp4"]["policy_version"] == "court-match-tennis-v2-loosecam"
...
def test_load_latest_identities_skips_explicit_settled_false_records(tmp_path):
    ...
    records.write_text(
        ...
        '"policy_version":"court-match-tennis-v2-loosecam","settled":true}\n'
        '{"item":"a.mp4","domain":"tennis","schema_version":"court-match-v1",'
        '"policy_version":"court-match-tennis-v2-loosecam","reason":"endpoint_error","settled":false}\n',
        encoding="utf-8",
    )
    identities = load_latest_identities(records)
    assert identities["a.mp4"]["policy_version"] == "court-match-tennis-v2-loosecam", (
...
def test_load_latest_identities_item_with_only_transient_records_has_no_identity(tmp_path):
    ...
    records.write_text(
        ...
        '"policy_version":"court-match-tennis-v2-loosecam","reason":"vlm_parse_failed","settled":false}\n'
        '{"item":"b.mp4","domain":"tennis","schema_version":"court-match-v1",'
        '"policy_version":"court-match-tennis-v2-loosecam","reason":"endpoint_error","settled":false}\n',
        encoding="utf-8",
    )
...
def test_load_latest_identities_falls_back_to_reason_code_when_settled_field_absent(tmp_path):
    ...
    records.write_text(
        '{"item":"a.mp4","domain":"tennis","schema_version":"court-match-v1",'
        '"policy_version":"court-match-tennis-v2-loosecam","reason":""}\n'
        '{"item":"a.mp4","domain":"tennis","schema_version":"court-match-v1",'
        '"policy_version":"court-match-tennis-v2-loosecam","reason":"endpoint_error"}\n',
        encoding="utf-8",
    )
    identities = load_latest_identities(records)
    assert identities["a.mp4"]["policy_version"] == "court-match-tennis-v2-loosecam", (
...
def test_resolve_todo_reproduction_from_finding_report():
    ...
    身份 (记录写入功能是本分支才加的)。按当前生效策略 (court-match-tennis-v2-loosecam),
