...
用法:
  SSHPASS='3dvision' python3 run_scene_split.py
  SSHPASS='3dvision' python3 run_scene_split.py --batch-size 300 --dry-run
  SSHPASS='3dvision' nohup python3 run_scene_split.py > logs/scene_split.log 2>&1 &
"""
import argparse
import os
import re
...
def push_batch(shm_out: str, workers_push: int = 4) -> tuple[int, float]:
    ...
    # 追加到审核队列（供 run_audit_splits.py 消费，无需列远端目录）
...
def main():
    ...
    if not os.environ.get("SSHPASS"):
        print("请设置 SSHPASS: SSHPASS='3dvision' python3 run_scene_split.py")
        sys.exit(1)
