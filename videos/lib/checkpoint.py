...
def load_latest_identities(records_path: Path) -> dict:
    """读 policy_records JSONL, 返回 {item: 最后一条记录的身份 dict}。
    「最后一条」= 文件中最后出现的记录 (JSONL 只追加, 后写覆盖前写的语义)。
...
def load_checkpoint(progress_path: Path, records_path: Path) -> dict:
    """合并「进度文件的文件名清单」与「JSONL 溯源记录的最新身份」。

    返回 {name: identity_dict_or_None}:
      - name 在进度文件中出现, 且在 records 中有对应的最新身份记录 -> 该身份 dict;
      - name 在进度文件中出现, 但 records 中没有 (legacy/unversioned 进度) -> None。
...
def is_current(identity, domain) -> bool:
    ...
    进度, 或从未被 policy_records 记录过) 视为不一致 -> 需要显式重新审核/迁移。"""
    if identity is None:
        return False
...
def resolve_todo(all_names, checkpoint: dict, domain) -> dict:
    ...
    - stale:   在 checkpoint 中但身份不是当前生效身份的条目 (旧策略判过/legacy未记录);
               todo 是 stale 的父集之一, 单独列出供调用方打日志/统计迁移规模,
               不代表额外多审一次。
