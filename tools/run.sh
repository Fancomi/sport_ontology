...
# python3 1_translate_wiki.py $VLM

# # 2. VLM 扩写视频描述（--check 启用 LLM 质检，质检复用同一组端口）
# python3 2_augment_wiki.py $VLM --check

# 2.1 批量校验 augment_xxx_cn.json 合规性 [已合入2混合调用]
# python3 2_1_check_augment.py $VLM
...
# python3 2_2_translate_augment.py $VLM --check --dry-run --limit 2
# for i in $(seq 1 5); do
#     python3 2_2_translate_augment.py $VLM --check
# done
...
# # #    清理过期键、补充新词、保留已有条目
# python3 5_enrich_with_llm.py $VLM --lang $LANG

# # # 5.1 基于 LLM 清理 slot_ontology_{lang}.json 中不恰当的混淆关系。
# python3 5_1_clean_ontology.py $VLM --lang $LANG
...
# #    --mode all         全部（默认）
# python3 8_eval_confusable.py $VLM --lang $LANG
# python3 8_eval_confusable.py $VLM --lang $LANG --mode confusable

# 评测 HARD, 读取hard_all_$LANG.jsonl
# python3 8_eval_confusable.py $VLM --lang $LANG --mode hard
...
# python3 8_3_cloze_eval.py $VLM --lang $LANG --dry-run --limit 4
# python3 8_3_cloze_eval.py $VLM --lang $LANG --limit 4
#  python3 8_3_cloze_eval.py $VLM \
#     --lang $LANG  --mode hard \
