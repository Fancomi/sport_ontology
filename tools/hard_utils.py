"""hard_utils.py — hard_all_{lang}.jsonl 共享工具。

被 9_extract_errors.py 和 9_1_clean_hard.py 共同引用，不含业务逻辑。

hard_all_{lang}.jsonl 是唯一权威源；hard_{view}.json 已从流水线中移除。
"""
...
def slotted_desc(video: str, view: str, lang: str = 'cn') -> str:
    """缓存 augment_{view}_{lang}.json 的 category_3_slotted_description。"""
    aug = DATA_ROOT / video / augment_name(view, lang)
    if not aug.exists():
        return ""
    try:
...
def key_valid(key: tuple, lang: str = 'cn') -> bool:
    video, view, slot, orig, _ = key
    return f"[{slot}:{orig}]" in slotted_desc(video, view, lang)
...
def clean_stale(hist: dict[tuple, dict], lang: str = 'cn') -> tuple[dict, int]:
    """删除 augment 中原始槽位值已消失的过期条目。"""
    clean = {k: v for k, v in hist.items() if key_valid(k, lang)}
