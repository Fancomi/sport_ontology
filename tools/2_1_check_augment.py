...
【各槽含义】
- gender: 性别（男性/女性）
- camera_view: 观察者视角（正面/侧面/斜侧面等）
- equipment: 器械（杠铃/哑铃/无器械等）
- contact_part: 身体接触部位（双手/双脚/背部等）
- contact_type: 接触方式（正握/反握/踩地/点地等）
- posture_alignment: 身体姿态/对齐状态（腰背挺直/双脚与肩同宽等）
- trajectory: 动作轨迹（向心上升/离心下降/顶峰收缩等）
- exercise: 动作专有名词（硬拉/弯举/深蹲等）
- force_part: 视觉可见发力/收缩部位（二头肌/背阔肌等）
- force_type: 发力方式（拉/推/保持/下蹲等）
- laterality: 被摄者解剖学左右侧（左侧/右侧/双侧/交替）

【检查项】
A. 槽位键必须在上述11个中，不得有拼写错误（如 posture_part、exercises）或自创键。
B. 槽位值必须为中文，不得为英文或梵文。
C. 描述内容须与 metadata_cn 一致（动作名、器械、发力肌群等），不得矛盾。
D. 串位检查：某槽的值不应是另一槽的语义（如 [equipment:推] 或 [force_type:哑铃]）。

【输出格式】仅输出 JSON，不含 markdown 或任何说明：
合格：{"pass": true}
不合格：{"pass": false, "issues": ["问题描述1", ...], "corrected": "修正后的完整文本（仅 category_3_slotted_description 内容）"}
"""
